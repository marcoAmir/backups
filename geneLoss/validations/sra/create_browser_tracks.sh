...
do
	...
	if [ $startPos -eq 0 ]; then
		...
		zcat -f $currBed | grep ${chr} | awk -v startPos=${startPos} -v endPos=${endPos} -F'\t' '{if($2>=startPos && $2<endPos)}'> ${outFile}
		zcat -f $currBed | grep ${chr} | awk -v startPos=${startPos} -v endPos=${endPos} -F'\t' '{if($3>startPos && $3<=endPos)}'>> ${outFile}
		zcat -f $currBed | grep ${chr} | awk -v startPos=${startPos} -v endPos=${endPos} -F'\t' '{if($2<=startPos && $3>=endPos)}'>> ${outFile}
		sort -u ${outFile} > tmp; mv tmp ${outFile}
	fi
done < SRR_Acc_List.txt

bedSort ${outFile} tmp.bed
./bed2wig.py -i tmp.bed --type bed -o tmp.wig

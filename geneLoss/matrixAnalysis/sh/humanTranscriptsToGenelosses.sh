...
callTable=$data/hg19.calltable
callTablePlacental=$data/hg19.calltable.placentals
...
then
	...
	if [ $speciesQuantileLoF -ne 0 ]; then
		$src/py/txScoringByQuantile.py $callTablePlacentalUnique $data/ quantileOutput -quantThrs $meanQuantileThrs
		callTablePlacentalUnique=quantileOutput
	fi
	...
	if [ $speciesQuantileLoF -ne 0 ]; then
		$src/py/txScoringByQuantile.py $callTable $data/ quantileOutput -quantThrs $meanQuantileThrs
		callTable=quantileOutput
	fi
	echo $callTable
